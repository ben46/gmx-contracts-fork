...
async function validateVaultBalance(expect, vault, vaultreader, token, offset) {
  if (!offset) { offset = 0 }
  const poolAmount = await vaultreader.poolAmounts(token.address)
  const feeReserve = await vaultreader.feeReserves(token.address)
