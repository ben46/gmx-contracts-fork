...
describe("Vault.buyUSDG", function () {
  ...
  it("buyUSDG", async () => {
    ...
    expect(await vaultReader.feeReserves(bnb.address)).eq(0)
    expect(await vault.getUsdgAmounts(bnb.address)).eq(0)
    expect(await vault.getPoolAmounts(bnb.address)).eq(0)

    expect(await vaultReader.whitelistedTokens(bnb.address)).eq(true)
    ...
    expect(await vaultReader.feeReserves(bnb.address)).eq(1)
    expect(await vault.getUsdgAmounts(bnb.address)).eq(29700)
    expect(await vault.getPoolAmounts(bnb.address)).eq(100 - 1)

    // console.log(bnb.address)
    await validateVaultBalance(expect, vault, vaultReader, bnb)
  ...
  it("buyUSDG allows gov to mint", async () => {
    ...
    expect(await vaultReader.feeReserves(bnb.address)).eq(0)
    expect(await vault.getUsdgAmounts(bnb.address)).eq(0)
    expect(await vaultReader.poolAmounts(bnb.address)).eq(0)
    ...
    expect(await vaultReader.feeReserves(bnb.address)).eq(1)
    expect(await vault.getUsdgAmounts(bnb.address)).eq(29700)
    expect(await vaultReader.poolAmounts(bnb.address)).eq(100 - 1)
  ...
  it("buyUSDG uses min price", async () => {
    ...
    expect(await vaultReader.feeReserves(bnb.address)).eq(1)
    expect(await vault.getUsdgAmounts(bnb.address)).eq(19800)
    expect(await vaultReader.poolAmounts(bnb.address)).eq(100 - 1)
  ...
  it("buyUSDG updates fees", async () => {
    ...
    expect(await vaultReader.feeReserves(bnb.address)).eq(30)
    expect(await vault.getUsdgAmounts(bnb.address)).eq(9970 * 300)
    expect(await vaultReader.poolAmounts(bnb.address)).eq(10000 - 30)
  ...
  it("buyUSDG uses mintBurnFeeBasisPoints", async () => {
    ...
    expect(await vaultReader.feeReserves(dai.address)).eq(expandDecimals(4, 18))
    expect(await vault.getUsdgAmounts(dai.address)).eq(expandDecimals(10000 - 4, 18))
    expect(await vaultReader.poolAmounts(dai.address)).eq(expandDecimals(10000 - 4, 18))
  })
  ...
  it("buyUSDG adjusts for decimals", async () => {
    ...
    expect(await usdg.balanceOf(user0.address)).eq(0)
    expect(await usdg.balanceOf(user1.address)).eq(0)
    expect(await vaultReader.feeReserves(btc.address)).eq(0)
    expect(await vault.getUsdgAmounts(bnb.address)).eq(0)
    expect(await vaultReader.poolAmounts(bnb.address)).eq(0)
    ...
    expect(await usdg.balanceOf(user0.address)).eq(0)
    expect(await vaultReader.feeReserves(btc.address)).eq(300000)
    expect(await usdg.balanceOf(user1.address)).eq(expandDecimals(60000, 18).sub(expandDecimals(180, 18))) // 0.3% of 60,000 => 180
    expect(await vault.getUsdgAmounts(btc.address)).eq(expandDecimals(60000, 18).sub(expandDecimals(180, 18)))
    expect(await vaultReader.poolAmounts(btc.address)).eq(expandDecimals(1, 8).sub(300000))

    await validateVaultBalance(expect, vault,vaultReader, btc)
  })
})
