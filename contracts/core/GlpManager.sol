...
import "../peripherals/VaultReader.sol";
...
contract GlpManager is ReentrancyGuard, Governable, IGlpManager {
    ...
    constructor(address _vault, address _usdg, address _glp, uint256 _cooldownDuration)  {
        gov = msg.sender;
        vault = IVault(_vault);
        usdg = _usdg;
        glp = _glp;
    ...
    function setVaultReader(address _vr) external onlyGov {
        vreader = VaultReader(_vr);
    }
    ...
    function getAum(bool maximise) public view returns (uint256) {
        ...
        bytes4 sel_getAllWhitelistedTokens = bytes4(keccak256("getAllWhitelistedTokens(address)"));

        for (uint256 i = 0; i < length;) {
            address token ;
            {
                bytes memory data_getAllWhitelistedTokens = abi.encodeWithSelector(sel_getAllWhitelistedTokens, i);
                (, bytes memory returnedData) = address(vault).staticcall(data_getAllWhitelistedTokens);
                token =  abi.decode(returnedData, (address));
            }
            
            /*
            address token = vault.getAllWhitelistedTokens(i);
            */
            bool isWhitelisted = vreader.whitelistedTokens(token);
            if (isWhitelisted)   {
                uint256 price = maximise ? vault.getMaxPrice(token) : vault.getMinPrice(token);
                uint256 poolAmount = vreader.poolAmounts(token);
                uint256 decimals = vreader.tokenDecimals(token);
    
                if (vreader.stableTokens(token)) {
                    aum = aum.add(poolAmount.mul(price).div(10 ** decimals));
                } else {
                    // add global short profit / loss
                    uint256 size = vault.getGlobalShortSizes(token);
                    if (size > 0) {
                        uint256 averagePrice = vault.getGlobalShortAveragePrices(token);
                    ...
                    aum = aum.add(vault.getGuaranteedUsd(token));

                    uint256 reservedAmount = vault.getReservedAmounts(token);
