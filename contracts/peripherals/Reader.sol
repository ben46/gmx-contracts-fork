...
contract Reader {
    ...
    uint256 public constant USDG_DECIMALS = 18;
     ...
     function getMaxAmountIn(IVault _vault, address _tokenIn, address _tokenOut) public view returns (uint256) {
         ...
         uint256 tokenInDecimals = _vault.getAddrObj(_tokenIn).tokenDecimals;
         uint256 tokenOutDecimals = _vault.getAddrObj(_tokenOut).tokenDecimals;

         uint256 amountIn;

         {
             uint256 poolAmount = _vault.getPoolAmounts(_tokenOut);
             uint256 reservedAmount = _vault.getReservedAmounts(_tokenOut);
             uint256 bufferAmount = _vault.getBufferAmounts(_tokenOut);
         ...
         uint256 maxUsdgAmount = _vault.getMaxUsdgAmounts(_tokenIn);
     ...
     /**    function getWhitelistedTokens(address _token) override external view returns (bool){
        return addrObjs[_token].whitelistedTokens;
    }

    function getStableTokens(address _token) override external view returns (bool){
        return addrObjs[_token].stableTokens;
    }

    function getShortableTokens(address _token) override external view returns (bool){
        return addrObjs[_token].shortableTokens;
    }

   
    function getTokenDecimals(address _token) override external view returns (uint256){
        return addrObjs[_token].tokenDecimals;
    }

    function getTokenWeights(address _token) override external view returns (uint256){
